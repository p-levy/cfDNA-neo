...
makeASCATObjectSegmentedOnlyOnBAFWithLogRCorrection <- function(ascat.bc) {
  ...
  #Here I take the
  rle.baf.pcf.df <- with(rle_w_na_handling(ascat.bc[["Tumor_BAF_segmented"]][[1]][,1] + as.numeric(ascat.bc$SNPpos$Chromosome[tf.vec.seg.baf.snps])), data.frame(number = values,
  ...
  rle.logr.pcf.df <- with(rle_w_na_handling(ascat.bc[["Tumor_LogR_segmented"]][,1]) , data.frame(number = values,
                                                                                                start = cumsum(lengths) - lengths + 1,
                                                                                                end = cumsum(lengths))[order(values),])
...
# References hg38
alleles38 <- paste0(ascat_ref_path, "/G1000_allelesAll_hg38/G1000_alleles_hg38_chr")
loci38 <- paste0(ascat_ref_path, "/G1000_lociAll_hg38/G1000_loci_hg38_chr") # Need to change chromosome name!! for i in {1..22} X; do sed -i 's/^/chr/' G1000_loci_hg38_chr${i}.txt; done
gc38 <- paste0(ascat_ref_path, "/GC_G1000_hg38.txt")
rt38 <- paste0(ascat_ref_path, "/RT_G1000_hg38.txt")
...
ascat.bc = ascat.loadData(Tumor_LogR_file = paste0(patient, "_cfDNA_tumourLogR.txt"), Tumor_BAF_file = paste0(patient, "_cfDNA_tumourBAF.txt"), Germline_LogR_file = paste0(patient, "_cfDNA_normalLogR.txt"), Germline_BAF_file = paste0(patient, "_cfDNA_normalBAF.txt"), gender = 'XX', genomeVersion = "hg38")
ascat.plotRawData(ascat.bc, img.prefix = "Before_correction_")
ascat.bc = ascat.correctLogR(ascat.bc, GCcontentfile = gc38, replictimingfile = rt38)
